 ...
 const renderCard = ({item, index}) => {
    return (
      <TouchableOpacity style={{marginHorizontal: 10, marginTop: 15}}>
            <TouchableOpacity
              onPress={() => navigation.navigate('ProductDetails', {storeId: item.id})}>
                <Image
          ...
          <View style={{flexDirection: 'row', marginTop: 15,borderRadius:5, marginLeft:30}}>
            <Text
              style={{color: '#17212A',fontWeight: 'bold', maxWidth: 170 }}>
              {item.name}
            </Text>
          </View>
                <Text style={{color: '#F97762',justifyContent:"center",marginLeft:30,marginTop:8}}>${item.price}</Text>
        </TouchableOpacity>
    )
  }
  ...
  return (
    <>
      <View style={{flex: 1, }}>
        {
          storeListLoading ?
          <Text style={{alignSelf: 'center', marginTop: 20, fontSize: 24}}>Loading...</Text>
          :
          <FlatList
            data={storeList}
            numColumns={2}
            renderItem={renderCard}
            ListEmptyComponent={() => <Text style={{alignSelf: 'center', marginTop: 20, fontSize: 24}}>No Store Found</Text>}
        />
        }
      </View>
    </>
