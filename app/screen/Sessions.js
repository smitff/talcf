    ...
    return (
      ...
      <View style={{ flex: 1, padding:16 }}>
        ...
        <View style={{ flexDirection: "row", }}>
           <Image style= {{width: "100%", height: 160, borderRadius: 10, backgroundColor: "red"}}
        ...
        <View style={{ flexDirection: "row", padding: 6, marginTop: 15,  width: '100%', backgroundColor: "#F5F5F5", }}>
          <View>
            <Text style={{ color: "#17212A",  marginTop: 5, }}>{item?.item?.title} </Text>
            <Text style={{  marginTop: 4 }}>{item?.item?.price}</Text>
          </View>
          <View style= {{ justifyContent:"center", position: "absolute",  right:10}}>
            <TouchableOpacity
            onPress={()=> navigation.navigate("SessionsDetails", {item: item})} 
            style={{  justifyContent:"center", alignItems:"center", 
            height: 40, width: 80,
             alignSelf:"flex-end", backgroundColor: "#F97762", 
             borderRadius: 6 ,  marginTop: 15,}}>
        ...
        <Text style={{ color: "#04A768", fontWeight: "bold", marginLeft: -310 }}>Free</Text>
      </View>
      </ScrollView>
    )
  }
  return (
    <View style={{ flex: 1, backgroundColor: '#f2f2f2', justifyContent: 'center', alignItems: 'stretch' }}>
      <View style={{ flexDirection: "row", marginBottom: 20 }}>
      </View>
      <FlatList data={dashboard?.course} renderItem={renderItem} keyExtractor={(_, index) => index.toString()} />
    </View>
  );
};
