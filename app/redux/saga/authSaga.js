...
export function* forgotPasswordWorker(action) {
  console.log("🚀 ~ file: authSaga.js ~ line 112 ~ function*forgotPasswordWorker ~ forgotPasswordWorker")
  yield put({ type: LOADINNG, payload: { status: true } });
...
const verifyOtpAPI = (body) => {
  ...
  return axios.post(URL+'verify_code' , {
    email: 'abc@abc.com',
...
export function* verifyOtpWorker(action) {
  ...
  console.log("🚀 ~ file: authSaga.js ~ line 165 ~ function*verifyOtpWorker ~ body", body)
  try {
...
const createpasswordAPI = (body, token,userId) => {
  ...
  return axios.post(URL+'create_new_password' , {
    user_id: userId,
    password: 12345678,
...
export function* dashboardWorker(action) {
  console.log("🚀 ~ file: authSaga.js ~ line 266 ~ function*dashboardWorker ~ dashboardWorker")
  try {
...
export function* sessiondetailsWorker(action) {
  try {
    ...
    if (res.data.data.responseCode == "200") {
      ...
      yield put({ type: SESSIONSDETAILS_RESPONSE, payload: res.data.data.payload });
      yield put({ type: LOADINNG, payload: { status: false } });
    } else {
      yield put({ type: LOADINNG, payload: { status: false } });
    }
...
const changePasswordApi = (body, token, userId) => {
  console.log("🚀 ~ file: authSaga.js ~ line 601 ~ changePasswordApi ~ body", body)
  const option = {
    headers: {
      Authorization: "Bearer " + token
    }
  }
  return axios.post(URL + 'change_password', {
    user_id: userId,
...
export function* changePasswordWorker(action) {
  try {
    ...
    console.log("🚀 ~ file: authSaga.js ~ line 613 ~ function*changePasswordWorker ~ token", token)
